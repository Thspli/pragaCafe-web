...
export default function Page() {
  ...
  React.useEffect(() => {
    if (talhoes.length > 0) fetchTotalArmadilhas();
  }, [talhoes]);

  React.useEffect(() => {
    const handleArmadilhaChange = () => { fetchTotalArmadilhas(); };
    window.addEventListener('armadilha:changed', handleArmadilhaChange);
    return () => { window.removeEventListener('armadilha:changed', handleArmadilhaChange); };
  }, [talhoes]);
    ...
    try {
      ...
      for (const talhao of talhoes) {
        ...
        if (res.ok) {
          ...
          const armadilhasValidas = uniqueArmadilhas.filter((a: any) => a.latitude != null && a.longitude != null);
  ...
  const handleConfirmArmadilha = async (data: any) => {
    try {
      ...
      if (data.existingId) {
        ...
        window.dispatchEvent(new CustomEvent('armadilha:changed', { detail: { action: 'updated', armadilha: updated, talhaoId: data.talhaoId } }));
      } else {
        const created = await createArmadilha(data, token);
        window.dispatchEvent(new CustomEvent('armadilha:changed', { detail: { action: 'created', armadilha: created, talhaoId: data.talhaoId } }));
      }
  ...
  const fetchRealArmadilhaCount = async (talhaoId: number) => {
    try {
      ...
      const armadilhas = await res.json();
      const uniqueArmadilhas = Array.from(new Map(armadilhas.map((a: any) => [a.id, a])).values());
      const armadilhasComCoordenadas = uniqueArmadilhas.filter((a: any) => a.latitude != null && a.longitude != null);
      setArmadilhaRealCount(armadilhasComCoordenadas.length);
    } catch (err) {
      console.error('❌ Erro ao buscar contagem de armadilhas:', err);
    }
  };
  ...
  const handleConfirmNovoTalhao = async () => {
    ...
    try {
      await createTalhao({
        ...
        center: tempPolygon.center, boundary: tempPolygon.boundary,
      });
    ...
    return (
      <div style={{
        ...
        <div style={{
          ...
          borderTop: "4px solid #8B4513", borderRadius: "50%", animation: "spin 1s linear infinite",
        }} />
        <p style={{ color: "#4A2C2A", fontWeight: 600 }}>☕ Carregando talhões da API...</p>
    ...
    <div style={{ height: "100vh", display: "flex", flexDirection: "column", overflow: "hidden" }}>
      ...
      {error && (
        <div style={{
          ...
          padding: "1rem 1.25rem", borderRadius: "0.5rem", border: "1px solid #C8860A",
          zIndex: 3000, maxWidth: "400px", boxShadow: "0 4px 12px rgba(0,0,0,0.15)",
        }}>
          ⚠️ {error}
        </div>
      )}
      ...
      <Header
        ...
        onNovoTalhao={() => {}}
        onListaTalhoes={() => setShowListaTalhoes(true)}
        onListaArmadilhas={() => setShowListaArmadilhas(true)}
        onMinhaLocalizacao={() => {}}
        onCreateTestTalhao={() => {}}
      />
      ...
      <ListaTalhoesModal
        ...
        talhoes={talhoes}
        onTalhaoClick={(talhao) => { handleTalhaoClick(talhao); setShowListaTalhoes(false); }}
        onNovoTalhao={() => { setShowListaTalhoes(false); setShowTutorial(true); }}
      />
      ...
      <ListaArmadilhasModal
        ...
        talhoes={talhoes}
        onArmadilhaClick={(armadilha) => { setArmadilhaSelecionada(armadilha); setShowListaArmadilhas(false); }}
      />

      <NovoArmadilhaModal
        open={armadilhaModalOpen}
        onClose={() => { setArmadilhaModalOpen(false); setPendingArmadilha(null); }}
